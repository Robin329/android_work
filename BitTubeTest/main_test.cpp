...
int main() {
  ...
  if (fork()) {
    ...
    if (size < 0)
      ...
      printf("\033[32mprocess(%d) send success, object size = %zd\n", getpid(),
             size);
    ...
    if (size < 0) {
      ...
      printf("\033[32mprocess(%d) receive success, object size = %zd\n",
             getpid(), size);
      for (int i = 0; i < size; ++i) {
        printf("\033[32mprocess(%d): id=%d, message=%d\n", getpid(),
               events[i].id, events[i].message);
      }
    }
    ...
    if (size < 0) {
      ...
      printf("\033[31mprocess(%d) receive success, object size = %zd\n",
             getpid(), size);
      for (int i = 0; i < size; ++i) {
    ...
    if (size < 0)
      ...
      printf("\033[31mprocess(%d) send success, object size = %zd\n", getpid(),
             size);
    sleep(2);
  }
