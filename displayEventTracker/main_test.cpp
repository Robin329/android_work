/**
 * Author Robin, Written in 2022/12/08
 */
...
int main(int argc, char **argv) {
  ...
  sp<DisplayEventTracker> displayEventTracker =
      new DisplayEventTracker(mVsyncRate);
  displayEventTracker->run("DisplayEventTracker", PRIORITY_DISPLAY);

  do {
    char input = getchar();
    if (input == 'q') break;
    if (input == 'r') displayEventTracker->requestNextVsync();
